class HTMLNode:
    ...
    def props_to_html(self):
        ...
        prop_str = prop_str + " ".join(
            [f"{key}" for key, value in self.props.items() if value is None]
        )
        prop_str = " " + prop_str
...
class LeafNode(HTMLNode):
    ...
    def to_html(self):
        if self.props:
            ...
            return f"<{self.tag}{html_str}>{self.value}</{self.tag}>"
...
class ParentNode(HTMLNode):
    ...
    def to_html(self):
        ...
        if html_props:
            html_str = f"<{self.tag}{html_props}>"
        else:
            html_str = f"<{self.tag}>"
